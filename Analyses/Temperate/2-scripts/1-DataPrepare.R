...
## Import data ----
load("G:/Delivery/R&DDel/HortForestSc/Horticulture/STRWBERY/BREED/BS22000/2024-25/2 Temperate 2024-25/1 Wandin/Wandin 2024-25 trials/Wandin data 2024-25/Analyses/Input/t2124-clean-spatial.RData")
d0 <- t2124_spatial
...
# number of rows is the same for all matrices (except dominance)

## Subset phenotypic data for traits of interest -------------------------------
sort(unique(d0$Trait))
yldtraits <- levels(d0$Trait)[grep("yldpp|avfrtwt", levels(d0$Trait))]
trait2keep <- yldtraits[grep("_w|_endMay", yldtraits)]
...
d1$HarvestWeek <- factor(d1$HarvestWeek, levels = c(paste0("0", 4:9), 10:22, "endMay"))
